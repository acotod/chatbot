...
export default function FlujoSPage() {
  ...
  function createBaseFlow() {
    ...
    const graph = buildBaseFlowGraph();
  ...
  async function startBaseFlow() {
    ...
    const flow = await createFlow.mutateAsync({ nombre: "Flujo base", tenantId: firstTenantId });
    const graph = buildBaseFlowGraph();
    setSelectedFlowId(flow.id);
    setRfNodes(graph.nodes);
    setRfEdges(graph.edges);
          ...
          <div className="flex-1 bg-white rounded-xl border p-4 overflow-y-auto space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setActiveTab("probar")} disabled={rfNodes.length === 0 || !selectedFlowId}
                  className="flex items-center gap-1 text-sm bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg disabled:opacity-40 transition">
                  <Play className="w-4 h-4" /> Ir a Probar
                </button>
              </div>
            </div>
            {rfNodes.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="rounded-xl border bg-slate-50 px-4 py-3">
                  <div className="text-xs text-slate-500">Pantallas</div>
                  <div className="text-lg font-semibold text-slate-900">{rfNodes.length}</div>
                </div>
                <div className="rounded-xl border bg-slate-50 px-4 py-3">
                  <div className="text-xs text-slate-500">Conexiones</div>
                  <div className="text-lg font-semibold text-slate-900">{rfEdges.length}</div>
                </div>
                <div className="rounded-xl border bg-slate-50 px-4 py-3">
                  <div className="text-xs text-slate-500">Siguiente paso</div>
                  <div className="text-sm font-medium text-slate-900">Prueba conversacional</div>
                </div>
              </div>
            )}
            {rfNodes.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { void startBaseFlow(); }} className="text-sm px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700">
                    Crear flujo base
                  </button>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b bg-[#0b141a] text-white">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setActiveTab("preview")} className="flex items-center gap-1 text-xs border px-3 py-1.5 rounded-lg hover:bg-gray-50">
                  <Phone className="w-3 h-3" /> Preview
                </button>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setActiveTab("probar")} disabled={!selectedFlowId}
                  ...
                  <Play className="w-3.5 h-3.5" /> Volver a Probar
                </button>
